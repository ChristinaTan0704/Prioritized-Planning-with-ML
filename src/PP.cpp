...
PP::PP(Instance& instance, int screen):
    instance(instance), screen(screen), path_table(instance.map_size), single_agent_planner(instance, 0)
{
    agents.reserve(instance.num_of_agents);
    for (int i = 0; i < instance.num_of_agents; i++)
    {
        agents.emplace_back(i, instance.start_locations[i], instance.goal_locations[i]);
    }
}
...
void PP::preprocess(bool compute_distance_to_start, bool compute_distance_to_goal,
                bool compute_mdd) // compute information in each agent
{
    assert(compute_distance_to_goal || !compute_mdd); // when compute_mdd is true, compute_distance_to_goal must be true
    clock_t start_time = clock();
    if (compute_distance_to_start)
    {
        for (auto& agent : agents)
        {
            agent.distance_to_start = instance.getDistances(agent.start_location);
        }
    }
    ...
    if (compute_mdd)
    {
        for (auto& agent : agents)
        {
            agent.mdd.buildMDD(instance, *agent.distance_to_goal, agent.start_location);
        }
    }
...
int PP::run(int& failed_agent_id, double time_out_sec)
{
    ...
    for (int id : ordering)
    {
        ...
        {
            if (agents[id].path.empty() || agent.second < (int)(agents[id].path.size()) - 1) // only consider agents that lead to shorter paths
        ...
        if (agents[id].path.empty())
        {
            sum_of_costs = MAX_COST;
            failed_agent_id = id;
            break; //failed, id is the failing agent. in its dependency graph, at least one pair should be reversed
        }
...
void PP::computeDefaultOrdering() // default ordering uses indices of the agents
{
...
void PP::computeRandomOrdering() // generate a random ordering
{
    ...
    std::random_shuffle(ordering.begin(), ordering.end()); // randomize the ordering
}

void PP::computeSHOrdering(bool randomRestart, double rr_beta) // prefer shorter start-goal shortest path
{
    if (ordering.size() < agents.size())
    {
        computeDefaultOrdering();
    ...
    {
        //softmax sampling
        //rr_base has to be negative because SH gives high priority to shorter distance
        ...
        vector<double> softmax_distances = softmax_vector<int>(all_start_goal_distances, rr_beta);
        double sum = std::accumulate(softmax_distances.begin(), softmax_distances.end(), 0.0);
        std::for_each(softmax_distances.begin(), softmax_distances.end(), [&sum, &target_sum](double& d) { d = d * target_sum / sum; });
...
void PP::computeLHOrdering(bool randomRestart, double rr_beta) // prefer longer start-goal shortest path
{
